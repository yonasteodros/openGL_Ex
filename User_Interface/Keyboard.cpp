 ...
 *
 *    Description:  mouse and keyboard input to glut
 *
 *        Version:  1.0
 *        Created:  06/18/2014 11:49:07
 *       Revision:  none
 *       Compiler:  gcc
 *
...
struct MyApp : public App{
  ...
  glm::vec3 forwardDir;                 //<-- forward looking vector
  ...
  void onDraw(){
     ...
     *-----------------------------------------------------------------------------*/
    glm::mat4 view = glm::lookAt( eyePos, eyePos+forwardDir, glm::vec3(0,1,0) );
    glm::mat4 proj = glm::perspective( PI/3.0f, (float)window().width()/window().height(), 0.1f, -10.0f);
    ...
    for (int i = 0; i < len; ++i){
        ...
        for (int j = 0; j < len; ++j){
            ...
            for (int k = 0; k < len; ++k){
              ...
              glm::vec3 axis((float)idx/total,1-(float)idx/total,0);
              float rad = time * PI;
              glm::vec3 vscale( .5  );
